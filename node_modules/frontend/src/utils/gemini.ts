/**
 * Gemini AI Utility — Centralized Google Gemini API integration
...
async function callGemini(
  ...
  try {
    const proxyRes = await fetch(`${BACKEND_URL}/api/gemini`, {
      method: 'POST',
  ...
  while (attempt < maxRetries) {
    try {
      ...
      if (res.status === 429 || res.status >= 500) {
        console.warn(`Gemini attempt ${attempt + 1}: status ${res.status}, retrying in ${delay}ms...`);
        await sleep(delay);
        delay *= 2;
        attempt++;
        continue;
...
/**
 * General-purpose Gemini prompt. Returns plain text.
 */
...
/**
 * Gemini prompt that parses the response as JSON.
 */
export async function askGeminiJSON<T = unknown>(prompt: string, systemInstruction?: string): Promise<T> {
  ...
  const text = await callGemini(prompt, systemWithJson);
  const cleaned = text.replace(/```(?:json)?\s*\n?/g, '').replace(/\n?```\s*$/g, '').trim();
  return JSON.parse(cleaned) as T;
}

/**
 * Gemini with Google Search grounding — for real-time data lookup.
 */
export async function askGeminiWithSearch(prompt: string, systemInstruction?: string): Promise<string> {
  return callGemini(prompt, systemInstruction, { useSearch: true });
}
...
Return JSON with:
{
  ...
  return askGeminiJSON<AIRiskAnalysis>(prompt, 'You are an expert Indian banking risk analyst. Analyze the loan application using RBI guidelines and return a structured risk assessment.');
}
...
Return JSON with:
{
  ...
  return askGeminiJSON<AIComplianceResult>(prompt, 'You are an expert Indian banking regulatory compliance officer. Check the loan against all applicable RBI norms.');
}
...
Return JSON with:
{
  ...
  return askGeminiJSON<AICounterOffer>(prompt, 'You are an expert Indian bank loan officer specializing in competitive pricing strategy.');
}
...
Return JSON with:
{
  ...
  return askGeminiJSON<AILoanExplanation>(prompt, 'You are an AI explainability expert helping bank customers understand loan decisions.');
}
...
Return JSON with:
{
  ...
  return askGeminiJSON<AIRatePrediction>(prompt, 'You are an AI interest rate prediction model for an Indian bank.');
}
...
Return JSON with:
{
  ...
  return askGeminiJSON<AIFraudAnalysis>(prompt, 'You are a fraud detection AI for an Indian bank. Analyze the data for synthetic identity, data mismatch, geographic anomaly, and other fraud patterns.');
}
...
Return JSON with:
{
  ...
  return askGeminiJSON<AIInvestmentAdvice>(prompt, 'You are a SEBI-registered investment advisor AI for Indian markets.');
}
...
export async function chatWithSearchGrounding(
  ...
  return callGemini(message, systemInstruction, { useSearch: true });
}
